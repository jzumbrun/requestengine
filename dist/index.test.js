...
const requestengine = (0, _1.initRequestEngine)({
  helpers: [{
    functions: {
      trim: (str) => str.trim(),
      eq: (a, b) => a === b,
  ...
  (0, globals_1.describe)('middleware', () => {
    ...
    (0, globals_1.it)('$users.update', done => {
      requestengine
        ...
        .then(() => {
          (0, globals_1.expect)(res.data.requests[0].results).toEqual(['UPDATE users SET $1 = $2, $3 = $4', ['name', 'Jon', 'age', 33]])
          done()
        })
        .catch(error => {
          done(error)
  ...
  (0, globals_1.describe)('execute', () => {
    ...
    (0, globals_1.it)('async handler', done => {
      requestengine
        ...
        .then(({ requests }) => {
          (0, globals_1.expect)(requests).toEqual([{ name: 'handler', results: 1 }])
          done()
        })
        .catch(error => {
          done(error)
